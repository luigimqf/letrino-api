...
export class AttemptRepository {
  ...
  static async create({ userId, statisticId, wordId, result }: IAttemptCreate): Promise<Either<Errors, Attempt>> {
    try {
  ...
  static async findByStatisticId(statisticId: string): Promise<Either<Errors, Attempt[]>> {
    try {
  ...
  static async countDocuments(conditions: IAttemptConditions): Promise<Either<Errors, number>> {
    try {
      const queryBuilder = this.repository.createQueryBuilder('attempt');

      if (conditions.userId) {
        queryBuilder.andWhere('attempt.userId = :userId', { userId: conditions.userId });
      }

      if (conditions.wordId) {
        queryBuilder.andWhere('attempt.wordId = :wordId', { wordId: conditions.wordId });
      }

      if (conditions.statisticId) {
        queryBuilder.andWhere('attempt.statisticId = :statisticId', { statisticId: conditions.statisticId });
      }

      if (conditions.result) {
        queryBuilder.andWhere('attempt.result = :result', { result: conditions.result });
      }
  ...
  static async findTodaysAttempts(userId: string): Promise<Either<Errors, Attempt[]>> {
    try {
  ...
  static async find(conditions: IAttemptConditions): Promise<Either<Errors, Attempt[]>> {
    try {
