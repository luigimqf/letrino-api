  ...
  async attemptSuccess(req: AuthenticateRequest, res: Response) {
    try {
      ...
      if(todaysWord.isSuccess() && todaysWord.value?.word === attempt) {
        ...
        let scoreCalculated = ATTEMPT_SCORES[Math.min(currentAttempt, 6) as keyof typeof ATTEMPT_SCORES] || 0;
        ...
        ok(res, { 
          ...
          bonuses: {
            ...
            streak: currentStreak >= 5 ? (currentStreak >= 10 ? BONUS_SCORES.STREAK_5 : BONUS_SCORES.STREAK_10) : null,
            highWinRate: winRate >= HIGH_WIN_RATE_THRESHOLD && totalGames >= 10 ? BONUS_SCORES.HIGH_WIN_RATE : 0
          }
        });
        return;
      }
